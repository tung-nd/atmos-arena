...
class WindowForecastingModule(LightningModule):
    ...
    def __init__(
        ...
        eta_min: float = 1e-8,
        windows = [[56, 112], [112, 168]],
        backup_log_path = '/localhome/prateiksinha/atmos-arena/atmos_arena/s2s/backup_log',
    ):
        super().__init__()
    ...
    def set_transforms(self, inp, out):
        def reverse_parameters(mean, std):
            std_denorm = 1 / std
            mean_denorm = -mean * std_denorm
            return mean_denorm, std_denorm
        
        self.inp_transform = transforms.Normalize(inp.mean, inp.std)
        self.out_transform = transforms.Normalize(out.mean, out.std)
        self.rev_inp_transform = transforms.Normalize(*reverse_parameters(inp.mean, inp.std))
        self.rev_out_transform = transforms.Normalize(*reverse_parameters(out.mean, out.std))
    ...
    def get_num_steps(self, window_in_days, lead_time):
        num_hours = 24 * window_in_days
        num_steps = num_hours // lead_time
        return int(num_steps)
    
    def test_step_direct(self, batch: Any, batch_idx: int):
    ...
    def get_loss_dict(self, x, y, out_variables):
        metrics=[lat_weighted_mse_val, lat_weighted_rmse, lat_weighted_mean_bias]
        all_loss_dicts = [
            m(x, y, lambda x : x, vars=out_variables, lat=self.lat, clim=None, log_postfix="") for m in metrics
        ]
        loss_dict = {}
        for d in all_loss_dicts:
            for k in d.keys():
                loss_dict[k] = d[k].item()
        return loss_dict
    ...
    def test_step(self, batch: Any, batch_idx, windows=None, device=None):
        x, y, lead_times, variables, out_variables = batch # only x is normalized
        
        if not windows:
            windows = self.hparams.windows
        if not device:
            device = 'cpu'
        ...
        predictions = [self.rev_inp_transform(x)] # store the unnormalized predictions for determining metrics
        num_steps = max([max(w) for w in windows])
        for i in range(num_steps):
            pred_diff_norm = self.net(x, lead_times, variables, out_variables)
            pred_diff_norm = self.replace_constant(pred_diff_norm, variables)
            pred_diff_raw = self.rev_out_transform(pred_diff_norm)
            inp_raw = self.rev_inp_transform(x)
            pred = pred_diff_raw + inp_raw
        ...
        for w in windows:
            ...
            mean_predictions = torch.mean(predictions[:,start:end,:,:,:], dim = 1)
            mean_y = torch.mean(y[:,start:end,:,:,:], dim = 1)
            metric_dict[start] = self.get_loss_dict(
                mean_predictions, 
                mean_y, 
                out_variables
            )
            
        path = os.path.join(self.hparams.backup_log_path, f"{batch_idx:04d}.json")
        with open(path, 'w') as json_file:
            json.dump(metric_dict, json_file, indent=4)
        
        for start in metric_dict.keys():
            loss_dict = metric_dict[start]
            for var in loss_dict.keys():
                # the wandb logger is really temperamental
                self.log(
                    f"test/{start}/{var}",
                    loss_dict[var],
                    on_step=False,
