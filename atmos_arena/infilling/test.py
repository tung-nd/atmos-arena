...
def get_data_given_path(path, variables):
    ...
    mean = np.mean(regridded_np, axis=0)
    std = np.std(regridded_np, axis=0)

    return regridded_np, mean, std


class Dataset(torch.utils.data.DataLoader):
    def __init__(self, data_path, mask_ratio_range):
        data_path = data_path
        in_data, mean, std = get_data_given_path(data_path, in_variables)  # (T, H, W)
        self.mean = mean
        self.std = std
        ...
        self.in_data = torch.from_numpy(in_data)  # (T, H, W)
    ...
    def __getitem__(self, i):
        mask = self.get_mask()
        return (
            self.normalize(self.in_data[i : i + 1, :, :]) * mask,
            self.normalize(self.in_data[i : i + 1, :, :]),
            mask,
        )
    ...
    def get_mask(self):
        ...
        mask = np.random.choice(
            [0, 1], size=self.in_data.shape[1:], p=[mask_ratio, 1 - mask_ratio]
        )  # (H, W)
...
PATHS = {
    ...
    "model": "/localhome/tungnd/atmos_arena/monthly_infilling/",  # Loop through each folder, cd checkpoints, run epoch_x.ckpt
}
batch_size = 1
...
dataset = Dataset(PATHS["temp_regridded"], [])
...
for ratio in mask_ratio_range:
    ...
    dataset.mask_ratio_range = [ratio]
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size)

    ratio_log_path = os.path.join(log_dir, f"{(int(ratio*10)):02d}")
...
print(
    "Done! See `atmos_arena/infilling/get_final_metrics.ipynb` for final metrics for each model"
)
