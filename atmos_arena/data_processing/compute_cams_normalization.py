...
if not os.path.exists(os.path.join(save_dir, f"normalize_mean.npz")):
    normalize_mean = {}
    normalize_std = {}
    normalize_log_mean = {}
    normalize_log_std = {}
    ...
    normalize_mean = np.load(os.path.join(save_dir, f"normalize_mean.npz"))
    normalize_std = np.load(os.path.join(save_dir, f"normalize_std.npz"))
    normalize_log_mean = np.load(os.path.join(save_dir, f"normalize_log_mean.npz"))
    normalize_log_std = np.load(os.path.join(save_dir, f"normalize_log_std.npz"))
...
for var in tqdm(list_single_vars + list_pressure_vars, desc='variables', position=0):
    for year in tqdm(years, desc='years', position=1, leave=False):
        if var in list_pressure_vars:
            ds = xr.open_dataset(os.path.join(root_dir, var, f'{year}.nc'))
        else:
    ...
    if var in SINGLE_LEVEL_VARS:
        ...
        np.savez(os.path.join(save_dir, f"normalize_mean.npz"), **normalize_mean)
        np.savez(os.path.join(save_dir, f"normalize_std.npz"), **normalize_std)
        ...
        if var in CHEMISTRY_VARS:
            ...
            np.savez(os.path.join(save_dir, f"normalize_log_mean.npz"), **normalize_log_mean)
            np.savez(os.path.join(save_dir, f"normalize_log_std.npz"), **normalize_log_std)
    else:
