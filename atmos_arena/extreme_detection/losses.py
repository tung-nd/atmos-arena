...
def weighted_jaccard_loss(logits, true, eps=1e-7):
    ...
    weights = torch.tensor([0.678, 31.08, 2.9], device=logits.device) # Tensor of size [ 3 ]
...
def weighted_cross_entropy_loss(logits, true):
    ...
    wce_loss = nn.CrossEntropyLoss(weight=torch.tensor([0.355, 72.171, 5.875], device=logits.device))
...
def weighted_jaccard_loss(logits, true, eps=1e-7):
    ...
    weights = torch.tensor([0.355, 72.171, 5.875], device=logits.device) # Tensor of size [ 3 ]
