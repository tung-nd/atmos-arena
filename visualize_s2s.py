...
def plot(model, dataset, out_denormalize):
    ...
    pred = model(x.cuda(), lead_time_tensor.cuda(), in_vars, out_vars).cpu()
    ...
    if lead_time == 336:
        lead_time_name = 'Weeks 3-4'
    elif lead_time == 672:
        lead_time_name = 'Weeks 5-6'
    column_titles = ['IC (00:00:00 Jan 1st 2020)', f'{lead_time_name} Ground truth', f'{lead_time_name} Forecast', 'Bias']
    ...
    plt.tight_layout()
    plt.savefig(f'{model_name}_{lead_time_name}_forecast.pdf', bbox_inches='tight')
    plt.close()
    # Show plot
    # plt.show()
...
def main(model_path):
    ...
    data_root_dir = '/eagle/MDClimSim/tungnd/data/wb2/1.40625deg_from_full_res_1_step_6hr_h5df_s2s'
    ...
    in_transform = get_normalize(data_root_dir, in_variables)
    out_transform = get_normalize(data_root_dir, out_variables)
    out_denormalize = get_denormalize(out_transform)
    
    dataset = ERA5WindowDataset(
        root_dir=os.path.join(data_root_dir, 'test'),
        clim_path=clim_path,
